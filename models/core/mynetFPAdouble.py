...
# )   with FPA bug batch=1 nn.functional input bug,and couple arch cost too much memory
...
class Mynet(nn.Module):
    def __init__(self, channel, filters=[64, 128, 256, 512, 1024]):
        ...
        self.fpa_bridge = FPA(channels=filters[3])
        ...
        self.output_layerFPA = nn.Sequential(nn.Conv2d(filters[1], 1, 1), nn.Sigmoid())
        ...
        self.conv1 = nn.Conv2d(filters[2]*4, filters[1]*10, 1)
    ...
    def forward(self, x):
        ...
        x4r = self.residual_conv3(x4)  #([1, 512, 64, 64])
        # print(x4r.shape)

        # x5 = self.aspp_bridge(x4r)
        # print(x5.shape) ([1, 1024, 64, 64])
        x5 =self.fpa_bridge(x4r)
        # print(x5.shape)
        x6 = self.attn1(x3r, x5)
        # print(x6.shape) ([1, 1024, 64, 64])  FPA 512,64
        x6 = self.upsample1(x6)
        # print(x6.shape) ([1, 1024, 128, 128]) FPA 512,128
        gau2 = self.gau_2(x4r, x4)  # 1, 256, 128, 128
        merge1 = torch.cat([gau2, x6, x4], dim=1)  # 1536,128,128  FPA 1024,128
        ...
        merge3 = torch.cat([gau4, x8, x2], dim=1)  # 1,384,512
        ...
        # x9 = self.aspp_out(x8)
        x9 = self.fpa_out(x8)
        # print(x9.shape) ([1, 64, 512, 512])  FPA128
        # out = self.output_layer(x9)
        out1 = self.output_layerFPA(x9)
        # print(out.shape) ([1, 1, 512, 512])
        input2= x1 *out1    #64.512
        # print(input2.shape)

        x2_ = self.squeeze_excite1(input2)
        # print(x2.shape) ([1, 64, 512, 512])
        ...
        # x5 = self.aspp_bridge(x4r)
        # print(x5.shape) ([1, 1024, 64, 64])
        x5_ = self.fpa_bridge(x4r_)
        # print(x5.shape)
        x6_ = self.attn1(x3r_, x5_)
        # print(x6.shape) ([1, 1024, 64, 64])  FPA 512,64
        x6u_ = self.upsample1(x6_)
        # print(x6.shape) ([1, 1024, 128, 128]) FPA 512,128
        gau2_ = self.gau_2(x4r_, x4_)  # 1, 256, 128, 128
        merge1 = torch.cat([gau2_, x6u_, x4_], dim=1)  # 1536,128,128  FPA 1024,128
        x6m_ = self.conv1(merge1)
        # print(x6.shape)
        # x6 = torch.cat([x6, x3r], dim=1)
        # print(x6.shape) ([1, 1280, 128, 128])
        x6__ = self.up_residual_conv1(x6m_)
        # print(x6.shape) ([1, 512, 128, 128])

        x7_ = self.attn2(x2r_, x6__)
        # print(x7.shape) ([1, 512, 128, 128])
        x7u_ = self.upsample2(x7_)
        # print(x7.shape) ([1, 512, 256, 256])
        gau3_ = self.gau_3(x3r_, x3_)  # 1,128,256
        merge2 = torch.cat([gau3_, x7u_, x3_], dim=1)  # 1,768,256
        x7m_ = self.conv2(merge2)
        # x7 = torch.cat([x7, x2r], dim=1)
        # print(x7.shape) ([1, 640, 256, 256])
        x7__ = self.up_residual_conv2(x7m_)
        # print(x7.shape) ([1, 256, 256, 256])

        x8_ = self.attn3(input2, x7__)
        # print(x8.shape) ([1, 256, 256, 256])
        x8u_ = self.upsample3(x8_)
        # print(x8.shape) ([1, 256, 512, 512])
        gau4_ = self.gau_4(x2r_, x2_)  # 1,64,512
        merge3 = torch.cat([gau4_, x8u_, x2_], dim=1)  # 1,384,512
        x8m_ = self.conv3(merge3)
        # x8 = torch.cat([x8, x1], dim=1)
        # print(x8.shape) ([1, 320, 512, 512])
        x8__ = self.up_residual_conv3(x8m_)
        # print(x8.shape) ([1, 128, 512, 512])

        # x9 = self.aspp_out(x8)
        x9 = self.fpa_out(x8__)
        # print(x9.shape) ([1, 64, 512, 512])  FPA128
        # out = self.output_layer(x9)
        out2 = self.output_layerFPA(x9)
