...
def conv3x3(in_planes, out_planes, stride=1):
    ...
    return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=True)#注意Bias为true的时候不要BN


class BasicBlock(nn.Module):#论文中权重共享怎么体现？
    expansion = 1
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        #self.bn1 = nn.BatchNorm2d(planes) 当设置bias为true的时候不要进行BN，因为BN中分母是方差操作
...
class Bottleneck(nn.Module):#没用到？
    expansion = 4
...
class LadderBlock(nn.Module):
    ...
    def __init__(self, planes, layers, kernel=3,block=BasicBlock,inplanes = 3):#layer=4
        super().__init__()
        self.planes = planes
        self.layers = layers
        self.kernel = kernel
...
class LadderNet(nn.Module):
    def __init__(self, inplanes=3, num_classes=1, layers=4, filters=10):
        super().__init__()
...
def test():
    ...
    model =LadderNet(inplanes=1,num_classes=1)
    predicts =model(x)
...
#dummy_input = torch.rand(1, 1, 64, 64).cuda()#假设输入1张64*64的图片


#with SummaryWriter(comment='LadderNet') as w:
